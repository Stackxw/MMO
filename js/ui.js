const UI = {
    ...
    async handleDownload(song) {
        if (!song) {
            this.showToast('暂无可下载歌曲', 'warning');
            return;
        }

        let target = song;
        if (!target.url) {
            this.showToast('正在获取下载链接...', 'info');
            try {
                target = await MusicAPI.getSongDetails(target);
            } catch (e) {
        ...
        const downloadUrl = MusicAPI.getProxyUrl(target.url, target.source);
        const title = (target.title || 'music').replace(/[\\/:*?"<>|]/g, '_');
        const artist = (target.artist || 'unknown').replace(/[\\/:*?"<>|]/g, '_');
        ...
        try {
            ...
            console.warn('Blob download failed, fallback to open url:', e);
            const opened = window.open(downloadUrl, '_blank', 'noopener');
            if (!opened) {
                this.showDialog({ title: '提示', content: '浏览器阻止了弹窗，请允许当前站点弹窗后重试。', showCancel: false });
                return;
            }
            this.showToast('已在新标签页打开下载链接', 'warning');
        }
    },
    ...
    showSongContextMenu(x, y, song) {
        ...
        const download = document.getElementById('ctx-download');
        const newPlayNext = playNext.cloneNode(true);
        const newAddPl = addPl.cloneNode(true);
        const newDownload = download.cloneNode(true);
        ...
        newDownload.onclick = async () => {
            menu.style.display = 'none';
            await this.handleDownload(song);
        };
    },
